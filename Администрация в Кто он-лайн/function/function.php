...
function encode_attach($sourse, $mod) {
	...
	$match_count = preg_match_all("#\[attach=([a-zA-Zа-яА-Я0-9\_\-\. ]+) align=([a-zA-Zа-яА-Я0-9\_\-\.\"\ ]+) title=([a-zA-Zа-яА-Я0-9\_\-\.\"\ ]+)\]#si", $sourse, $date);
...
function user_sinfo() {
	...
	if ($conf['session']) {
		...
		if ($guest==3 && $module=='') $module='Админка';
			...
			if ($guest == 3) {
				$who_online .= "<tr><td>".user_geo_ip($host, 3)."</td><td><font color='red' title='Администратор'><b>$strip</b></font></td><td align=\"right\">$linkstrip</td></tr>";
				$a++;
			} elseif ($guest == 2) {

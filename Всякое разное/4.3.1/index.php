...
function account() {
	...
	if (!is_user()) {
		...
		if (extension_loaded("gd") && ($conf['gfx_chk'] == 2 || $conf['gfx_chk'] == 4 || $conf['gfx_chk'] == 5 || $conf['gfx_chk'] == 7)) {
			echo "<div class=\"left\">"._SECURITYCODE.":</div><div class=\"center\"><img src=\"captcha.php\" onclick=\"if(!this.adress)this.adress = this.src; this.src=adress+'?rand='+Math.random();\" border=\"1\" title=\"Нажмите, чтобы обновить картинку\" style=\"cursor:pointer;\" alt=\""._SECURITYCODE."\"></div>"
		...
		."<h5>[ <a href=\"index.php?name=".$conf['name']."&op=newuser\" title=\""._REGNEWUSER."\">"._REGNEWUSER."</a> ]</h5>";
		close();
		foot();
	} elseif (is_user()) {
		profil();
...
function newuser() {
	...
	if (!is_user()) {
		...
		if (!$confu['reg']) {
			...
			open();
			echo "<form action=\"index.php?name=".$conf['name']."\" method=\"post\">"
			."<div class=\"left\">"._NICKNAME.":</div><div class=\"center\"><input type=\"text\" name=\"user_name\" id=\"new_user_name\"  value=\"".$user_name."\" size=\"20\" maxlength=\"25\" class=\"rss_info\">&nbsp;<input  class=\"fbutton\" onclick=\"SlaedCheckLogin(); return false;\" type=\"button\"  value=\"Проверить имя\" /><div id='result-registration'></div></div>"
			...
			."<h5>[ <a href=\"index.php?name=".$conf['name']."\" title=\""._USERLOGIN."\">"._USERLOGIN."</a> ]</h5>";
			close();
		}
		foot();
	} elseif (is_user()) {
...
function info() {
	...
	$result = $db->sql_query("SELECT u.user_id, u.user_name, u.user_rank, u.user_email, u.user_website, u.user_avatar, u.user_regdate, u.user_icq, u.user_occ, u.user_from, u.user_interests, u.user_sig, u.user_viewemail, u.user_aim, u.user_yim, u.user_msnm, u.user_password, u.user_storynum, u.user_blockon, u.user_block, u.user_theme, u.user_newsletter, u.user_lastvisit, u.user_lang, u.user_points, u.user_last_ip, u.user_warnings, u.user_group, u.user_birthday, u.user_gender, u.user_votes, u.user_totalvotes, u.user_field, u.user_agent, g.name, g.rank, g.color, COUNT(c.id) FROM ".$prefix."_users AS u LEFT JOIN ".$prefix."_groups AS g ON (g.id=u.user_group) LEFT JOIN ".$prefix."_comment AS c ON (c.uid=u.user_id) WHERE user_name='$user_name'");

	if ($db->sql_numrows($result) > 0) {
		list($user_id, $user_name, $user_rank, $user_email, $user_website, $user_avatar, $user_regdate, $user_icq, $user_occ, $user_from, $user_interests, $user_sig, $user_viewemail, $user_aim, $user_yim, $user_msnm, $user_password, $user_storynum, $user_blockon, $user_block, $user_theme, $user_newsletter, $user_lastvisit, $user_lang, $user_points, $user_last_ip, $user_warnings, $user_group, $user_birthday, $user_gender, $user_votes, $user_totalvotes, $user_field, $user_agent, $gname, $grank, $gcolor, $commts) = $db->sql_fetchrow($result);
		head();
		open();
		if ($user_lastvisit!=0) { 
 	                $dltrus = new Date_DeltaRussian(); 
 		                $user_lastvisit=$dltrus->spellDelta(strtotime($user_lastvisit),time(),3)._TIME_PAST; 
 		                } else $user_lastvisit=_TIME_NOT_LOGIN;
		echo get_info($user_id, $user_name, $user_rank, $user_email, $user_website, $user_avatar, $user_regdate, $user_icq, $user_occ, $user_from, $user_interests, $user_sig, $user_viewemail, $user_aim, $user_yim, $user_msnm, $user_password, $user_storynum, $user_blockon, $user_block, $user_theme, $user_newsletter, $user_lastvisit, $user_lang, $user_points, $user_last_ip, $user_warnings, $user_group, $user_birthday, $user_gender, $user_votes, $user_totalvotes, $user_field, $user_agent, $gname, $grank, 1, $gcolor, $commts);
		close();
...
function last($uid, $name) {
	...
	if ($db->sql_numrows($result) > 0) {
		...
		while(list($id, $cid, $modul, $date, $comment) = $db->sql_fetchrow($result)) {
			...
			$massiv = array(_QUOTE, _HIDE, "PHP - "._CODE, _CODE);
                  foreach ($massiv as $val){
			$comment = preg_replace("#".$val."#i", " ", $comment);
			}
	...
	$result = $db->sql_query("SELECT sid, title, time FROM ".$prefix."_stories WHERE uid='".$user_id."' AND time <= now() AND status!='0' ORDER BY sid DESC LIMIT 0,10");
	if ($db->sql_numrows($result) > 0) {
		open();
		echo "<table width=\"100%\" border=\"2\" cellpadding=\"3\" cellspacing=\"1\" align=\"center\" class=\"bgcolor4\">"
...
function passlost() {
	...
	if (!is_user()) {
		...
		open();
		echo "<form action=\"index.php?name=".$conf['name']."\" method=\"post\">"
		."<div class=\"left\">"._EMAIL.":</div><div class=\"center\"><input type=\"text\" name=\"email\" value=\"".$email."\" maxlength=\"355\" size=\"45\" class=\"".$conf['style']."\"></div>";
...
function login() {
	...
	$result = $db->sql_query("SELECT user_id, user_name, user_email, user_password, user_storynum, user_blockon, user_theme FROM ".$prefix."_users WHERE user_name='$uname' AND user_password='".md5($upass)."'");
	...
	if (!$user_id || $user_name != $uname || $user_password != md5($upass)) $stop = _LOGININCOR;
	if (!$stop) {
		...
		if ($conf['forum']) {
			new_user($user_name, $user_password, $user_email);
			log_in($uname, $upass);
		}
...
function edithome() {
	...
	if (is_user()) {
		...
		."<div class=\"left\">"._BIRTHDAY.": </div><div class=\"center\" style=\"white-space: nowrap;\">".datetime(2, "user_birthday", $userinfo['user_birthday'], 10, 60, 380, $conf['style'])."</div>"
		...
		if ($confu['news'] == 1) {
			echo "<div class=\"left\">"._C_12.":</div><div class=\"center\"><select name=\"user_storynum\" class=\"".$conf['style']."\">";
			echo "<option value=\"3\"";
			...
			echo ">"._C_12." - 10</option>";
			echo "<option value=\"15\"";
			if ($userinfo['user_storynum'] == "15") echo " selected";
			echo ">"._C_12." - 15</option>";
			echo "<option value=\"20\"";
			if ($userinfo['user_storynum'] == "20") echo " selected";
			echo ">"._C_12." - 20</option></select></div>";
		} else {
			echo "<input type=\"hidden\" name=\"user_storynum\" value=\"".$confn['newnum']."\">";
		}
		echo "<div class=\"left\">"._RNEWSLETTER."</div><div class=\"center\">".radio_form($userinfo['user_newsletter'], "user_newsletter")."</div>"
		."<div class=\"left\">"._ALLOWUSERS."</div><div class=\"center\">".radio_form($userinfo['user_viewemail'], "user_viewemail")."</div>"

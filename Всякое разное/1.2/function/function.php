...
function ajax_rating($typ, $id, $mod, $rat, $scor, $obj="") {
	...
	if (($con[1] && $id && $mod) || ($rat && $scor)) {
		$content = (($con[1] && $typ) || ($con[1] && !$con[2] && !$typ)) ? "<div id=\"rep".$id.$obj."\" OnClick=\"if(!this.loprotect){LoadGet('1', '".$id.$obj."', '1', 'rating', '".$id."', '', '".$obj."', '".$mod."', '');this.loprotect=1;}\" OnDblClick=\"LoadGet('1', '".$id.$obj."', '1', 'rating', '".$id."', '', '".$obj."', '".$mod."', '');\" class=\"rate\">".vote_graphic($rat, $scor)."</div>" : "<div class=\"rate\">".vote_graphic($rat, $scor)."</div>";
		return $content;
	}
}
...
function head() {
	...
	if (!is_admin() && !defined("ADMIN_FILE") && $conf['close']) {
        if ($conf['prof'])
          get_exit($conf['close-prof'], 0);
        else get_exit(_CLOSE_TEXT, 0);
      }
	if ($conf['session']) {
		$ip = getip();

...
function get_theme() {
	...
	if (isset($_GET['shkl']) && file_exists("templates/".text_filter($_GET['shkl'])."") && $_GET['shkl'] != "") $theme=text_filter($_GET['shkl']);
	return $theme;
}
...
else $content = (($con[1] && $typ) || ($con[1] && !$con[2] && !$typ)) ? "<script type=\"text/javascript\">ajax('ajax.php?go=1&op=rating&mod=".$mod."&id=$id', 'rate".$id."');</script><div class=\"rate\" id=\"rate".$id."\"></div>" : "<div class=\"rate\">".vote_graphic($rat, $scor)."</div>";		return $content;
	}
}
...
function ashowcom() {
	...
	if ($numstories > 0) {
		...
		foreach ($cmassiv as $val) {
			...
			#$thank = "<a href=\"javascript: scroll(0, 0);\" title=\""._THANK."\"><img src=\"".img_find("forum/$currentlang/thank")."\" border=\"0\" alt=\""._THANK."\" title=\""._THANK."\"></a>";
